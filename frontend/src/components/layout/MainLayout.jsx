...
function MainLayout() {
  ...
  const workspaceLinks = [
    ...(roles.includes('ADMINISTRATOR')
      ...
      : []),
    ...(roles.some((role) => role === 'MODERATOR' || role === 'ADMINISTRATOR')
      ? [{ to: '/moderation', label: 'Kiểm duyệt', icon: '✓' }]
      : []),
    ...(roles.includes('RELIC_MANAGER')
      ? [{ to: '/manage/quizzes', label: 'Quản lý quiz', icon: '?' }]
      : []),
    ...(roles.includes('BUSINESS_OWNER')
      ? [{ to: '/business-analytics', label: 'Kinh doanh', icon: '↗' }]
      : []),
  ]

  const isWorkspacePath = /^(\/admin|\/moderation|\/manage\/quizzes|\/business-analytics)/.test(location.pathname)
  const showWorkspace = Boolean(user && isWorkspacePath && workspaceLinks.length)
                ...
                {userOpen && (
                  <div className="user-menu__panel" role="menu">
                    <div className="user-menu__summary">
                      ...
                      <span>{roles[0]?.replaceAll('_', ' ') || 'Thành viên'}</span>
                    </div>
                    ...
                    {workspaceLinks.length > 0 && <div className="user-menu__separator" />}
          ...
          <aside id="mobile-navigation" className="mobile-nav" aria-label="Menu di động">
            <div className="mobile-nav__header">
              <NavLink className="brand" to="/">
                ...
                <span className="brand__copy"><strong>LTSS</strong><small>Khám phá Sơn Tây</small></span>
              </NavLink>
        ...
        <div className="workspace-shell page-container">
          <aside className="workspace-nav" aria-label="Điều hướng khu vực quản lý">
            <div className="workspace-nav__heading">
              ...
              <strong>{roles.includes('ADMINISTRATOR') ? 'Quản trị LTSS' : 'Quản lý LTSS'}</strong>
            </div>
            {workspaceLinks.map((link) => (
              <NavLink key={link.to} className={workspaceLinkClassName} to={link.to}>
                <span aria-hidden="true">{link.icon}</span>{link.label}
              </NavLink>
            ))}
            <div className="workspace-nav__spacer" />
            <NavLink className="workspace-nav__link" to="/">
              <span aria-hidden="true">←</span>Về trang công khai
            </NavLink>
          </aside>
